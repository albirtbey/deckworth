...
interface EditTradePageProps {
    ...
    updateTradeListing: (listingId: string, listingData: Partial<TradeListing>) => void;
}

const DROPPABLE_AREA_COLLECTION = 'userCollectionForListingMulti';
const DROPPABLE_AREA_LISTING_CARDS = 'listingCardsArea';

export function EditTradePage({ currentUser, collection, tradeListings, updateTradeListing }: EditTradePageProps) {
    ...
    useEffect(() => {
        if (!currentUser) {
            toast.error("You must be logged in to edit a trade listing.");
            navigate('/login');
            return;
        }
        ...
        if (listing.userId !== currentUser.id) {
            toast.error("You can only edit your own trade listings.");
            navigate('/my-trades');
            return;
        }

        if (listing.status !== 'Active') {
            toast.error("You can only edit active trade listings.");
            navigate('/my-trades');
            return;
        }

        // Initialize form with existing data
        setCardsInListing(listing.cards);
        setAskingPrice(listing.askingPrice?.toString() || '');
        setWishlistItems(listing.wishlist?.join(', ') || '');
        setDescription(listing.description || '');
    }, [currentUser, listingId, tradeListings, navigate]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        updateTradeListing(listingId, updatedListingData);
        toast.success(`Trade listing updated successfully!`);
        navigate('/my-trades');
    };
    ...
    return (
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
            <div className="container py-8">
                <Button variant="ghost" onClick={() => navigate('/my-trades')} className="mb-6 text-sm">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <UICard>
                            <CardHeader>
                                <CardTitle>Edit Trade Listing</CardTitle>
                            </CardHeader>
                            <CardContent>
